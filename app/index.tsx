...
export default function MainPage() {
  ...
  const filteredData = DATA.filter((item) => {
    ...
    if (selectedFilter === 'Tracked by AI') return matchesSearch && item.trackedByAI;
    if (selectedFilter === 'Not Tracked by AI') return matchesSearch && !item.trackedByAI;
  ...
  const renderItem = ({ item }: any) => (
    <View style={styles.card}>
      ...
      <TouchableOpacity style={styles.addButton}>
        <AntIcons name='plus' size={calcSize(20)} color={theme.primary}/>
      </TouchableOpacity>
    </View>
  );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity hitSlop={10} onPress={() => router.push('/settings')}>
          <Icons name='settings' size={calcSize(30)} color={theme.primary}/>
        </TouchableOpacity>
      </View>
